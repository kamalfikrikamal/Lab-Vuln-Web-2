...
source "virtualbox-iso" "teknobantu" {
  ...
  boot_command = [
    ...
    " hostname=teknobantu domain=local",
    "<enter>"
  ]
  ...
  format            = "ova"
  output_directory  = "packer/output/teknobantu"
}

build {
  sources = ["source.virtualbox-iso.teknobantu"]
  ...
  provisioner "file" {
    source      = "packer/build-context/teknobantu-src.tar.gz"
    destination = "/tmp/teknobantu-src.tar.gz"
  }
